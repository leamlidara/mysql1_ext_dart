...
//          values.addAll(doubleToList(value));
  ...
  int _measureBlob(Blob value, dynamic preparedValue) {
    return Buffer.measureLengthCodedBinary(preparedValue.length) + (preparedValue.length as int);
  }

  void _writeBlob(Blob value, dynamic preparedValue, Buffer buffer) {
    log.fine('BLOB: $value');
    buffer.writeLengthCodedBinary(preparedValue.length);
    buffer.writeList(preparedValue);
  }
  ...
  int _measureString(String value, dynamic preparedValue) {
    return Buffer.measureLengthCodedBinary(preparedValue.length) + (preparedValue.length as int);
  }

  void _writeString(String value, dynamic preparedValue, Buffer buffer) {
